input <- readLines("2022/day11_input.txt")

item <- lapply(strsplit(gsub(".+: ", "", input[0:7*7 + 2L]), ", "), as.integer)
op <- lapply(gsub(".+=", "\\\\(old)", input[0:7*7 + 3L]), \(x) eval(parse(text = x)))
div <- as.integer(gsub("\\D", "", input[0:7*7 + 4L]))
tru <- as.integer(gsub("\\D", "", input[0:7*7 + 5L])) + 1L
fal <- as.integer(gsub("\\D", "", input[0:7*7 + 6L])) + 1L
